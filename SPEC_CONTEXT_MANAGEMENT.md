...
| MCP | `src/plugins/mcp-manager.ts`, `src/plugins/mcp-transport.ts`, `agent.mcp.example.json` | stdio / streamable-http / sse → `mcp_<server>_<tool>`，`allow`/`deny` 过滤 |
...
| MCP | `@modelcontextprotocol/sdk` | stdio / streamable-http / sse，工具名 `mcp_<server>_<tool>` |
