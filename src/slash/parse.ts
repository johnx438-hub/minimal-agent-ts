/**
 * Shared slash command **parse** layer (TUI + Web + future UIs).
 * No TUI overlays, no Runtime — pure string → SlashResult.
 */

import type { MemorySlashAction } from '../workspace-memory.js';
import { parseMemorySlash } from '../workspace-memory.js';

/** Help/autocomplete locale (TUI zh|en maps here; 'both' for tests). */
export type SlashLocale = 'zh' | 'en' | 'both';
...
function formatHint(entry: SlashHelpEntry, locale: SlashLocale = 'both'): string {
