...
> **状态**: Draft v0.1（2026-07-15）· **W1 待实现**  
...
- [ ] `preset: "dev-worker"` 的 workflow step 工具集与 spawn 同 preset 一致（含 strip 规则）  
- [ ] 旧 `prompt_file: "../roles/…"` 的 workflow 仍能跑  
- [ ] 单测：resolve 合并、tools 覆盖、forbidden tools strip  
- [ ] `review-loop` 示例改为 preset 引用且 `npm test` 相关用例绿  
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| **W1** | `resolveAgentProfile`；workflow `preset`；路径 cwd+兼容；shell 策略；示例迁移 | ⏳ |
