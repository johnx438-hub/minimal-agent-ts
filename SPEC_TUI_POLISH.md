...
| 内容 | `session · model · shell/web · jobs:N · turn:N · [io] · Σm · Σs · ctx · c:hit%` |
| Token 分项 | **Σm** / **Σs** = 主/spawn billed；**ctx** = 主 Agent 最近 `prompt_tokens`/limit；**c** = session 前缀 cache hit%（`llm_done.cache` 累计，无遥测时省略）；换 session 清零 |
