...
| 项 | 现状 | 规划 |
|----|------|------|
| 子 Agent `max_turns` | `spawn_policy.max_turns_default` 15，`max_turns_cap` 30 | 增加 **stress preset** 或 `spawn_policy.stress_max_turns`（如 50–100），仅显式 preset 启用 |
| 子 Agent shell | preset 含 `run_shell` 时继承父级 `allowShell` + JIT gate | **可行**；见 §5.3 |
| 并行度 | `max_parallel` 默认 1–2 | 压测配置 `max_parallel: 3` + 多 API profile 分散限流 |
| 工具集 | `agents/worker.md` 类预设 | 新增 `agents/dev-worker.md`：read/write/edit/grep/shell（可选） |
